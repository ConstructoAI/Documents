...
- **Aucun report automatique** des heures pointées vers les heures réelles des opérations d'un bon de travail (cette saisie reste manuelle au Module 11).
...
> **Un même écran, quatre modules backend.** Cette page traverse quatre routeurs distincts : `employees.py` (le cœur : feuilles de temps + résumé de paie), `payroll.py` (paie CCQ), les trois routeurs de feuillets, et `pointage_ai.py` (assistant). C'est la raison pour laquelle certains sujets renvoient au **Module 14 Comptabilité / Paie** (paie CCQ, feuillets) et au **Module 10 Employés** (taux horaire, NAS).
...
- **Capacité hebdomadaire** : le nombre d'heures visées par semaine pour un employé (par défaut **40 h**, réglé sur sa fiche au Module 10). Sert à colorer l'occupation (sous-charge, normal, dépassement).
...
La paie réelle, avec retenues détaillées, charges de l'employeur et CCQ. Le détail des taux et des paliers d'impôt est documenté au **Module 14 Comptabilité / Paie** ; voici la surface de cet onglet.
...
> Les pourcentages ci-dessus sont les **étiquettes affichées dans la fiche**. La gouvernance des taux (paliers d'impôt, plafonds, exemptions, CCQ par métier) relève du module Paie — voir le **Module 14**.
...
> Pour le détail des taux, des paliers d'impôt et de la CCQ, voir le **Module 14**.
...
> Les feuillets s'appuient sur la paie CCQ **générée** : sans fiches de paie pour la période, les sommaires restent vides. Le NAS et l'adresse de chaque employé (saisis au Module 10) sont nécessaires à un feuillet complet.
...
| RQAP | 0,494 % |
| AE | 1,32 % |
| Impôt fédéral / provincial | selon les paliers (Module 14) |
...
> Étiquettes telles qu'affichées dans le bulletin. Gouvernance des taux, plafonds et paliers : **Module 14**.
...
- Le **NAS** et l'**adresse** saisis au Module 10 sont nécessaires aux feuillets T4 / RL-1.
- Cet écran ne montre pas la fiche RH ; il ne fait qu'utiliser ces données.

### 5.2 Module 11 — Bons de travail
...
- **Aucun report automatique** : les heures pointées ne remplissent pas les « heures réelles » des opérations du bon de travail — cette saisie reste manuelle au Module 11.

### 5.3 Module 08 — Projets
...
### 5.4 Module 14 — Comptabilité / Paie / Feuillets
...
- L'assistant de ce module est **distinct** de l'Assistant IA général de l'ERP (Module 24) et des assistants comptables. Il est volontairement **cloisonné** : aucune requête libre en base, aucun montant de paie, aucune donnée nominative sensible. Pour toute analyse financière, se tourner vers les modules comptables et leurs contrôles.
...
**Manuels liés** :
- Module 10 (Employés — taux horaire, capacité, NAS, NIP mobile) — `10-operations-employes.md`
- Module 11 (Bons de travail — opérations, heures réelles) — `11-operations-bons-de-travail.md`
- Module 08 (Projets — coût de la main-d'œuvre, heures par projet) — `08-ventes-projets.md`
- Module 14 (Comptabilité / Paie — taux, paliers, feuillets T4 / RL-1 / PD7A) — `14-operations-comptabilite.md`
