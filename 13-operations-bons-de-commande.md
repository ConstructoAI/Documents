# Module 13 — Bons de commande (achats fournisseurs)
...
Les bons de commande peuvent aussi **naître d'une demande de prix** (RFQ), c'est-à-dire d'un appel d'offres à plusieurs fournisseurs. Ce cycle vit dans l'onglet voisin **« Demandes de prix »** (décrit au **Module 09**). Le point de jonction est l'**octroi** :
...
> Le bon **reste** au statut « Reçu » (pour l'audit) ; c'est l'**inventaire** que l'on corrige, par les contre-mouvements. Voir le Module 09 pour l'annulation des mouvements.
...
> Le serveur refuse de facturer deux fois le même bon (bon déjà facturé ou déjà comptabilisé), et refuse un bon sans ligne ni fournisseur. La facture créée reste **en brouillon** : elle se complète et se comptabilise dans le **Module 14 — Comptabilité**.
...
> Le module **ne facture rien** directement via Stripe ou QuickBooks. Le bouton « Créer facture » crée une facture **en brouillon** ; c'est la Comptabilité (Module 14) qui la comptabilise et, le cas échéant, l'exporte vers QuickBooks.
...
| Module | Lien |
|--------|------|
| **Magasin / Inventaire** (Module 09) | Les bons de commande **sont** l'onglet « Commandes » du Magasin. La réception d'un bon alimente le stock (mouvement d'entrée) ; la correction d'une réception passe par l'annulation des mouvements ; les articles proviennent du catalogue de produits ; les fournisseurs sont ceux de l'onglet « Fournisseur ». |
| **Demandes de prix** (Module 09) | L'**octroi** d'une demande de prix génère un bon de commande par fournisseur retenu ; ces bons alimentent l'onglet Commandes. |
| **Comptabilité** (Module 14) | Le bouton **Créer facture** génère une facture fournisseur brouillon ; par ailleurs, un bon transmis (au-delà du brouillon) avec un montant est **comptabilisé automatiquement** au grand livre à la synchronisation. La réception, via le mouvement de stock, alimente aussi la comptabilité d'inventaire. |
| **Projets** (Module 08) | Un bon peut être associé à un projet (colonne, filtre, tri). |
| **Dossiers / Fiche 360** (Module 06) | Un bon dont le projet est lié à une opportunité avec dossier est **rattaché au dossier** ; un bon s'ouvre directement depuis un dossier via `/magasin?open=<id>`. |
| **Employés** (Module 10) | L'employé responsable d'un bon est choisi parmi les employés actifs. |
| **Configuration** (Module 30) | Les taux de taxes (TPS / TVQ, provinces, États-Unis), le logo et le thème des documents proviennent de la Configuration de l'entreprise. |
| **Crédits IA** (Module 24) | Le scan IA d'un bon consomme les crédits prépayés du tenant. |
...
Un bon **Reçu**, **Facturé** ou déjà **comptabilisé** ne se supprime pas (traçabilité). Pour un bon comptabilisé, contre-passez d'abord son écriture au grand livre (Module 14). Un simple brouillon, lui, se supprime sans problème.
...
**Manuels liés** :
- Module 09 — Magasin (produits, inventaire, mouvements, demandes de prix, fournisseurs) — `09-operations-magasin.md`
- Module 11 — Bons de Travail (sorties de matériel vers le chantier) — `11-operations-bons-de-travail.md`
- Module 14 — Comptabilité (factures fournisseurs, écritures au grand livre) — `14-operations-comptabilite.md`
- Module 08 — Projets (association des bons à un projet) — `08-ventes-projets.md`
- Module 06 — Dossiers / Fiche 360 (ouverture directe d'un bon) — `06-ventes-dossiers.md`
- Module 30 — Configuration (taxes, thème des documents) — `30-configuration.md`
- Module 24 — Assistant IA (crédits IA du scan) — `24-communication-assistant-ia.md`
