# Module 10 — Employees (human resources)
...
> - Backend: `backend/routers/employees.py` (≈ 2,162 lines; employee record + SIN reveal + statistics; this same file also hosts the time-tracking endpoints `/time-entries*`, but those are driven by **Module 12 Time Tracking**, not by this screen), `backend/routers/employes_ai.py` (≈ 257 lines, read-only workforce assistant)
...
> **Scope**: this module is the **employee directory** — the human-resources record. It is used to **create, search, filter, edit and deactivate** employees, to **export the list to CSV** and to query an **AI workforce assistant**. The record also captures **mobile security** data (PIN, mobile role, stock-management permission) and **tax data** (encrypted Social Insurance Number, address) "for the T4 / RL-1 slips". It **does not enter** hours (that is done in **Module 12 Time Tracking**), it **does not calculate** payroll or tax slips (that is done in **Module 14 Accounting / Payroll**), and it **never deletes** an employee (only deactivation exists). Skills are **displayed** but **cannot be edited** here.
...
- **No hours entry or approval.** Time entries appear only **read-only** (the 5 most recent). All hours management (creation, approval, weekly timesheet, export, cost per project) is done in **Module 12 Time Tracking**.
...
- **Status**: an employee's state (Active, On leave, Training, Work stoppage, Inactive). Only **Active** employees enter the payroll runs of Module 14.
...
- **Weekly capacity**: the target number of hours per week (default 40). Module 12 uses it to flag an under-loaded or over-loaded week.
...
- **Recent time entries**: the **5 most recent** time entries (date + hours). No creation or approval here — that happens in Module 12.
...
> **After a change to the mobile app's sub-path, employees may have to re-subscribe to notifications** (see Module 12). The PIN itself remains valid.
...
> This data is used only for **payroll** and the **slips** produced in Accounting (Module 14). Entering it here is a prerequisite for generating the T4 / RL-1 slips.
...
4. Both sections are **read-only**. To enter hours, go to Module 12 Time Tracking; skills cannot be edited in the ERP.
...
> The four trades `STRUCTURE_BETON`, `CHARPENTE_BOIS`, `FINITION`, `MECANIQUE_BATIMENT` (together with `CHANTIER` and `ELECTRICITE`) determine, at payroll time (Module 14), whether the employee falls under the CCQ. Classifying the department correctly here is therefore useful downstream.

**5 statuses** (`STATUTS`, default **ACTIF**):

| Status | Badge color | Enters payroll (Module 14) |
|--------|-------------|-----------------------------|
...
> The **time-tracking** endpoints (`/employees/time-entries*`, `/employees/payroll-summary`) live in the same backend file but are **documented in Module 12 Time Tracking**. This screen uses them read-only for the "5 most recent time entries" only.
...
### 5.1 Module 12 — Time Tracking

- **Time tracking** (entry, approval, weekly timesheet, cost per project, CSV export of hours) is done in Module 12, at `/pointage`.
- The **weekly capacity** entered here feeds the under-load / over-load indicators of the Module 12 timesheet.
...
### 5.2 Module 14 — Accounting / Payroll
...
- The **workforce** assistant in this module is **separate** from the ERP's general AI Assistant (Module 24). It is deliberately **siloed**: no free-form database queries, no individual data. For any named or sensitive question, it directs the user to the Employees / Payroll module and its Law 25 controls.
...
A: In **Module 12 Time Tracking** (`/pointage`). This screen only shows the **5 most recent** time entries, read-only.

**Q: How do I produce the T4 / RL-1 slips?**
A: In **Accounting / Payroll** (Module 14). Here, you only **enter** the required SIN and address. Make sure these fields are filled in for each relevant employee before the production period.
...
A: On this record, the new rate applies to **future** time entries. Time entries **already** recorded keep the rate captured at the moment of the entry (Module 12 freezes that rate to avoid retroactively recalculating payroll).
...
- **Skills** and **5 most recent time entries**: **read-only** here (hours entry in Module 12, skills not editable).
...
**Related manuals**:
- Module 09 (Store — stock-management permission) — `09-operations-magasin.md`
- Module 12 (Time Tracking — hours entry and approval) — `12-operations-pointage.md`
- Module 14 (Accounting / Payroll — T4 / RL-1 / PD7A slips) — `14-operations-comptabilite.md`
- Module 24 (General ERP AI Assistant) — `24-communication-assistant-ia.md`
