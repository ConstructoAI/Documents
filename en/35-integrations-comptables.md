# Module 35 — Accounting Integrations (QuickBooks, Sage)
...
- **Accounting (module 15).** Import populates your **chart of accounts** and creates **invoices** in the ERP; export sends your invoices and payments to the accounting software. It is the module whose data flows the most through this connector.
...
- **Company management (module 04).** Your **customers** and **suppliers** are the "bidirectional" entities: they go to the accounting software and come back, without ever overwriting a locally modified name.
- **Configuration (module 28).** This module **is** a tab of Configuration. Configuration also hosts the **webhooks** used by the tab of the same name, and manages the **subscription**: a suspended subscription switches the module into **read-only (view) mode** (writes blocked). The **taxes** (GST/QST) set in Configuration determine the exported breakdown.
...
*Related manuals:* `28-configuration.md` (parent module: Integrations tab, webhooks, subscription, and read-only mode), `15-operations-comptabilite.md` (chart of accounts and invoices exchanged), `08-ventes-soumissions.md` and `09-ventes-projets.md` (exported sales invoices), `04-gestion-entreprises.md` (bidirectional customers and suppliers).
