...
- **Sales / Quotes (module 08).** The estimate creates a **budgetary quote** (project "Construction") that you open and finalize in the Sales module. The "Open quote" button leads there directly.
- **CAD / 3D Modeling (module 31).** The CAD file (XML) is meant to be **imported as needed** into the CAD module; this import remains **manual** (no automatic bridge as of today).
...
- **Configuration / Subscription (module 28).** In **consultation mode** (suspended subscription), all PDF3D writes are blocked.
...
*Related manuals:* `08-ventes-soumissions.md` (budgetary quote produced by the estimate), `31-outils-dao-modelisation.md` (CAD, same 3D DESIGN section, target of a future CAD import), `25-communication-assistant-ia.md` (AI estimating engine and prepaid credits), `28-configuration.md` (subscription and consultation mode).
