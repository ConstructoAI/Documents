# Module 13 — Time Tracking and Hours
...
- **No automatic carry-over** of tracked hours to the actual hours of a work order's operations (that entry stays manual in Module 12).
...
> **One screen, four backend modules.** This page crosses four distinct routers: `employees.py` (the core: time sheets + payroll summary), `payroll.py` (CCQ payroll), the three slip routers, and `pointage_ai.py` (assistant). That is why some topics point to **Module 15 Accounting / Payroll** (CCQ payroll, slips) and **Module 11 Employees** (hourly rate, SIN).
...
- **Weekly capacity** : the target number of hours per week for an employee (default **40 h**, set on their record in Module 11). Used to colour occupancy (under-load, normal, over-load).
...
- **"Weekly capacity per employee" card** : legend "Green < 80% · Yellow 80-100% · Red > 100% (overloaded)". Columns: **Employee**, **Hours worked**, **Capacity**, **Occupancy** (coloured percentage) and **Load** (progress bar). Default capacity is **40 h** (adjustable per employee in Module 11); an employee at 100% or more is flagged "overloaded".
...
Actual payroll, with detailed deductions, employer contributions and CCQ. The detail of the rates and tax brackets is documented in **Module 15 Accounting / Payroll**; here is the surface of this tab.
...
> The percentages above are the **labels displayed on the pay stub**. Rate governance (tax brackets, ceilings, exemptions, CCQ by trade) belongs to the Payroll module — see **Module 15**.
...
> For the detail of the rates, tax brackets and CCQ, see **Module 15**.
...
> The slips rely on the **generated** CCQ payroll: without pay records for the period, the summaries stay empty. Each employee's SIN and address (entered in Module 11) are required for a complete slip.
...
| RQAP | 0.494% |
| EI | 1.32% |
| Federal / provincial tax | per the brackets (Module 15) |
...
> Labels as displayed on the pay stub. Governance of rates, ceilings and brackets: **Module 15**.
...
- The **SIN** and **address** entered in Module 11 are required for the T4 / RL-1 slips.
- This screen does not show the HR record; it only uses that data.

### 5.2 Module 12 — Work Orders
...
- **No automatic carry-over** : tracked hours do not fill the "actual hours" of the work order's operations — that entry stays manual in Module 12.

### 5.3 Module 09 — Projects
...
### 5.4 Module 15 — Accounting / Payroll / Slips
...
- This module's assistant is **distinct** from the ERP's general AI Assistant (Module 25) and from the accounting assistants. It is deliberately **compartmentalized** : no free query against the database, no payroll amount, no sensitive personal data. For any financial analysis, turn to the accounting modules and their controls.
...
**Related manuals** :
- Module 11 (Employees — hourly rate, capacity, SIN, mobile PIN) — `11-operations-employes.md`
- Module 12 (Work Orders — operations, actual hours) — `12-operations-bons-de-travail.md`
- Module 09 (Projects — labour cost, hours by project) — `09-ventes-projets.md`
- Module 15 (Accounting / Payroll — rates, brackets, T4 / RL-1 / PD7A slips) — `15-operations-comptabilite.md`
- Module 25 (ERP general AI Assistant) — `25-communication-assistant-ia.md`
