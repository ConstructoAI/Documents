...
- **Module distinct** : l'Assistant IA a son propre serveur (`routers/ai.py`) et sa propre facturation en crédits IA — il ne fait **pas** partie de la section Aide et ressources. Voir `25-communication-assistant-ia.md`.
...
L'Assistant IA est la seule aide **assistée à l'intérieur** de l'ERP, mais il ne fait pas partie de cette section : c'est un module autonome (serveur `routers/ai.py`), atteignable par le bouton étincelles de la barre supérieure (`TopBar.tsx:312-320`). Il consomme des **crédits IA** et peut consulter vos données. Détails : `25-communication-assistant-ia.md`.
...
**Manuels liés** :
- Assistant IA (aide assistée in-app, crédits IA) — `25-communication-assistant-ia.md`
- Agent vocal (téléphonie, chatbot vocal) — `32-communication-agent-vocal.md`
- Configuration (abonnement, moyen de paiement, préférences) — `28-configuration.md`
- Web (recherche en direct + liens utiles Québec) — `27-outils-web.md`
