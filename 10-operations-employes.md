# Module 10 — Employés (ressources humaines)
...
> - Backend : `backend/routers/employees.py` (≈ 2 162 lignes ; fiche employé + révélation du NAS + statistiques ; ce même fichier héberge aussi les points d'accès de pointage `/time-entries*`, mais ceux-ci sont pilotés par le **Module 12 Pointage**, pas par cet écran), `backend/routers/employes_ai.py` (≈ 257 lignes, assistant d'effectif en lecture seule)
...
> **Frontière importante.** Ce module est la **gestion** des employés. Le **Pointage** des heures (saisie, validation, feuille de temps, export) est un module distinct (`/pointage`, Module 12). La **Paie** et les **feuillets T4 / RL-1 / PD7A** vivent dans la **Comptabilité** (Module 14). Le NAS et l'adresse sont **saisis ici**, mais ils ne servent qu'aux traitements de paie situés ailleurs.
...
- **Aucune saisie ni validation d'heures.** Les pointages n'apparaissent qu'en **lecture seule** (les 5 plus récents). Toute la gestion des heures (création, validation, feuille hebdomadaire, exportation, coût par projet) se fait au **Module 12 Pointage**.
...
- **Statut** : l'état d'un employé (Actif, Congé, Formation, Arrêt de travail, Inactif). Seuls les employés **Actifs** entrent dans les traitements de paie du Module 14.
...
- **Capacité hebdomadaire** : le nombre d'heures visées par semaine (par défaut 40). Le Module 12 s'en sert pour signaler une semaine en sous-charge ou en dépassement.
...
- **Pointages récents** : les **5 derniers** pointages (date + heures). Aucune création ni validation ici — cela se passe au Module 12.
...
> **Après un changement de sous-chemin de l'application mobile, les employés peuvent devoir se réabonner aux notifications** (voir Module 12). Le NIP, lui, reste valide.
...
> Ces données ne servent qu'à la **paie** et aux **feuillets** produits dans la Comptabilité (Module 14). Les saisir ici est un prérequis à la génération des T4 / RL-1.
...
4. Ces deux sections sont en **lecture seule**. Pour saisir des heures, aller au Module 12 Pointage ; les compétences ne s'éditent pas dans l'ERP.
...
> Les quatre métiers `STRUCTURE_BETON`, `CHARPENTE_BOIS`, `FINITION`, `MECANIQUE_BATIMENT` (avec `CHANTIER` et `ELECTRICITE`) déterminent, au moment de la paie (Module 14), si l'employé relève de la CCQ. Bien classer le département ici est donc utile en aval.

**5 statuts** (`STATUTS`, défaut **ACTIF**) :

| Statut | Couleur du badge | Entre en paie (Module 14) |
|--------|------------------|----------------------------|
...
> Les points d'accès de **pointage** (`/employees/time-entries*`, `/employees/payroll-summary`) résident dans le même fichier backend mais sont **documentés au Module 12 Pointage**. Cet écran ne les utilise qu'en lecture pour les « 5 derniers pointages ».
...
### 5.1 Module 12 — Pointage

- Le **pointage des heures** (saisie, validation, feuille de temps hebdomadaire, coût par projet, exportation CSV des heures) se fait au Module 12, à l'adresse `/pointage`.
- La **capacité hebdomadaire** saisie ici alimente les indicateurs de sous-charge / dépassement de la feuille de temps du Module 12.
...
### 5.2 Module 14 — Comptabilité / Paie
...
- L'assistant d'**effectif** de ce module est **distinct** de l'Assistant IA général de l'ERP (Module 24). Il est volontairement **cloisonné** : aucune requête libre en base, aucune donnée individuelle. Pour toute question nominative ou sensible, il renvoie l'utilisateur vers le module Employés / Paie et ses contrôles Loi 25.
...
R : Au **Module 12 Pointage** (`/pointage`). Cet écran ne montre que les **5 derniers** pointages, en lecture seule.

**Q : Comment produire les T4 / RL-1 ?**
R : Dans la **Comptabilité / Paie** (Module 14). Ici, vous ne faites que **saisir** le NAS et l'adresse nécessaires. Assurez-vous que ces champs sont remplis pour chaque employé concerné avant la période de production.
...
R : Sur cette fiche, le nouveau taux s'applique aux **futurs** pointages. Les pointages **déjà** enregistrés conservent le taux capté au moment du pointage (le Module 12 fige ce taux pour éviter de recalculer rétroactivement la paie).
...
- **Compétences** et **5 derniers pointages** : **lecture seule** ici (saisie des heures au Module 12, compétences non éditables).
...
**Manuels liés** :
- Module 09 (Magasin — droit de gérer le stock) — `09-operations-magasin.md`
- Module 12 (Pointage — saisie et validation des heures) — `12-operations-pointage.md`
- Module 14 (Comptabilité / Paie — feuillets T4 / RL-1 / PD7A) — `14-operations-comptabilite.md`
- Module 24 (Assistant IA général de l'ERP) — `24-communication-assistant-ia.md`
- Module 30 (Configuration — utilisateurs et rôles) — `30-configuration.md`
