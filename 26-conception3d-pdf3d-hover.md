...
- **Ventes / Soumissions (module 07).** L'estimation crée un **devis budgétaire** (projet « Construction ») que vous ouvrez et finalisez dans le module Ventes. Le bouton « Ouvrir le devis » y mène directement.
- **DAO / Modélisation 3D (module 25).** Le fichier CAD (XML) est prévu pour être **importé au besoin** dans le DAO ; cette importation reste **manuelle** (aucun pont automatique à ce jour).
...
- **Configuration / Abonnement (module 30).** En **mode consultation** (abonnement suspendu), toutes les écritures de PDF3D sont bloquées.
...
*Manuels liés :* `07-ventes-soumissions.md` (devis budgétaire produit par l'estimation), `25-outils-dao-modelisation.md` (DAO, même section CONCEPTION 3D, cible d'un import CAD futur), `24-communication-assistant-ia.md` (moteur d'estimation IA et crédits prépayés), `30-configuration.md` (abonnement et mode consultation).
