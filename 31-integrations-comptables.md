# Module 31 — Intégrations comptables (QuickBooks, Sage)
...
- **Comptabilité (module 14).** L'import remplit votre **plan comptable** et crée des **factures** dans l'ERP ; l'export envoie vos factures et paiements vers le logiciel comptable. C'est le module dont les données circulent le plus par ce connecteur.
...
- **Gestion des entreprises (module 03).** Vos **clients** et **fournisseurs** sont les entités « bidirectionnelles » : ils partent vers le logiciel comptable et en reviennent, sans jamais écraser un nom modifié localement.
- **Configuration (module 30).** Ce module **est** un onglet de la Configuration. C'est aussi la Configuration qui héberge les **webhooks** utilisés par l'onglet du même nom, et qui gère l'**abonnement** : un abonnement suspendu fait passer le module en **mode consultation** (écritures bloquées). Les **taxes** (TPS/TVQ) réglées dans la Configuration déterminent la ventilation exportée.
...
*Manuels liés :* `30-configuration.md` (module parent : onglet Intégrations, webhooks, abonnement et mode consultation), `14-operations-comptabilite.md` (plan comptable et factures échangés), `07-ventes-soumissions.md` et `08-ventes-projets.md` (factures de vente exportées), `03-gestion-entreprises.md` (clients et fournisseurs bidirectionnels).
