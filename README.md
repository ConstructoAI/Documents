# Documentation CONSTRUCTO AI

## Guide Utilisateur Complet

Bienvenue dans la documentation officielle de **CONSTRUCTO AI**, votre ERP intelligent conçu spécifiquement pour les entreprises de construction au Québec.

---

## Table des Matières

### Navigation & Tableau de Bord
| Module | Description | Lien |
|--------|-------------|------|
| Tableau de Bord | Vue d'ensemble et KPIs principaux | [01-tableau-de-bord.md](01-tableau-de-bord.md) |
| Analytics BI | Business Intelligence et analyses avancées | [02-analytics-bi.md](02-analytics-bi.md) |
| Projets | Gestion complète des projets de construction | [03-projets.md](03-projets.md) |

### Gestion Commerciale (CRM)
| Module | Description | Lien |
|--------|-------------|------|
| Entreprises | Gestion des clients et fournisseurs | [04-entreprises.md](04-entreprises.md) |
| Contacts | Répertoire des contacts professionnels | [05-contacts.md](05-contacts.md) |
| Ventes | Pipeline commercial et opportunités | [06-ventes.md](06-ventes.md) |
| Devis | Création et gestion des soumissions | [07-devis.md](07-devis.md) |

### Planification
| Module | Description | Lien |
|--------|-------------|------|
| Calendrier | Calendrier des événements et échéances | [08-calendrier.md](08-calendrier.md) |
| Gantt | Diagramme de Gantt pour la planification | [09-gantt.md](09-gantt.md) |
| Kanban | Tableau Kanban pour le suivi des tâches | [10-kanban.md](10-kanban.md) |

### Production & Ressources Humaines
| Module | Description | Lien |
|--------|-------------|------|
| Production | Gestion de la production et des opérations | [11-production.md](11-production.md) |
| Employés | Gestion des ressources humaines | [12-employes.md](12-employes.md) |
| TimeTracker | Pointage et suivi des heures | [13-timetracker.md](13-timetracker.md) |
| RBQ/CCQ | Conformité construction Québec | [14-rbq-ccq.md](14-rbq-ccq.md) |

### Achats & Inventaire
| Module | Description | Lien |
|--------|-------------|------|
| Produits | Catalogue de produits et matériaux | [15-produits.md](15-produits.md) |
| Achats | Gestion des achats et fournisseurs | [16-achats.md](16-achats.md) |
| Inventaire | Gestion des stocks | [17-inventaire.md](17-inventaire.md) |
| Logistique | Logistique et livraisons | [18-logistique.md](18-logistique.md) |

### Finance & Comptabilité
| Module | Description | Lien |
|--------|-------------|------|
| Comptabilité | Comptabilité complète double entrée | [19-comptabilite.md](19-comptabilite.md) |
| Fonds Prévoyance | Gestion Loi 16 copropriétés | [20-fonds-prevoyance.md](20-fonds-prevoyance.md) |
| Subventions | Subventions et aides gouvernementales | [21-subventions.md](21-subventions.md) |

### Communication
| Module | Description | Lien |
|--------|-------------|------|
| Emails | Gestion des courriels | [22-emails.md](22-emails.md) |
| Conférence | Visioconférence intégrée | [23-conference.md](23-conference.md) |
| Assistant IA | Assistant intelligent Claude | [24-assistant-ia.md](24-assistant-ia.md) |
| Prompts IA | Guide des prompts pour l'estimation | [25-prompt-ai.md](25-prompt-ai.md) |

### Administration
| Module | Description | Lien |
|--------|-------------|------|
| WEB | Outils de recherche web | [26-web.md](26-web.md) |
| Utilisateurs | Gestion des comptes utilisateurs | [27-utilisateurs.md](27-utilisateurs.md) |
| Configuration | Paramètres de l'entreprise | [28-configuration.md](28-configuration.md) |
| Abonnement | Gestion de l'abonnement Stripe | [29-abonnement.md](29-abonnement.md) |

### Documentation Technique
| Module | Description | Lien |
|--------|-------------|------|
| Analyse ERP | Comparatif des solutions ERP construction 2025 | [30-analyse-comparative-erp-2025.md](30-analyse-comparative-erp-2025.md) |
| Architecture Globale | Vue d'ensemble architecturale du système (8 domaines, 189+ tables) | [31-architecture-globale.md](31-architecture-globale.md) |
| API REST | Documentation complète de l'API (30+ endpoints, webhooks, exemples) | [32-api-documentation.md](32-api-documentation.md) |

### Guides Pratiques
| Guide | Description | Lien |
|-------|-------------|------|
| Qualification Prospects | Guide stratégique B.A.T., entonnoir 5 étapes, scripts et grilles | [33-guide-qualification-prospects.md](33-guide-qualification-prospects.md) |
| Subventions 2025 | 50+ programmes de subventions pour entrepreneurs au Québec | [34-guide-subventions-2025.md](34-guide-subventions-2025.md) |

---

## Premiers Pas

### 1. Connexion
1. Accédez à l'application via votre navigateur
2. Entrez le courriel de votre entreprise
3. Saisissez votre nom d'utilisateur et mot de passe
4. Cliquez sur "Se connecter"

### 2. Navigation
- Utilisez le **menu latéral gauche** pour naviguer entre les modules
- Le **tableau de bord** affiche vos KPIs en temps réel
- Les **notifications** apparaissent en haut de l'écran

### 3. Aide
- Chaque module dispose d'une section d'aide contextuelle
- L'**Assistant IA** peut répondre à vos questions en temps réel
- Contactez le support via la messagerie intégrée

---

## Support

Pour toute question ou assistance :
- **Assistant IA** : Disponible 24/7 dans l'application
- **Documentation** : Ce guide et les manuels détaillés
- **Support** : info@constructoai.ca
- **Téléphone** : (514) 820-1972

---

© 2025 Constructo AI - Tous droits réservés | Développé par Sylvain Leduc
