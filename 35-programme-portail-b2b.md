# Module 35 — Portail B2B / B2C (clients et fournisseurs)
...
| | **Portail client** (ce manuel) | **Back-office fournisseur** (module 05) |
|---|---|---|
...
> **À retenir :** il n'existe **aucun assistant IA dans le portail client**. L'assistant « Gestion B2B » que le fournisseur peut utiliser vit dans le back-office ERP (module 05) et n'est **jamais** accessible depuis votre portail. Aucun bouton, aucune fonction du portail ne consomme de crédits d'intelligence artificielle.
...
Ce workflow s'exécute **côté fournisseur** (module 05 — Ventes → onglet B2B/B2C), pas dans le portail client. Il est décrit ici pour que le client sache quoi demander :
...
- **Ventes / Back-office B2B (module 05).** C'est le **pendant fournisseur** du portail. Le fournisseur y **approuve** votre accès, **relie** votre compte à votre fiche d'entreprise (active le Suivi), **chiffre** vos demandes en soumissions, **crée** vos contrats, **fait avancer** vos commandes et **répond** à vos messages. L'« Assistant IA — Gestion B2B » y vit aussi, du côté du fournisseur uniquement.
- **Entreprises (module 03).** Votre **fiche d'entreprise** dans le CRM du fournisseur (`companies`) est ce à quoi votre compte de portail doit être **relié** pour que le Suivi montre vos devis et projets.
- **Soumissions / Devis (module 07).** Les **devis réels** visibles dans Suivi → Devis proviennent de ce module. Le **lien externe** d'une carte ouvre la **page publique** du devis (`/devis/public/{jeton}`), seul document consultable et imprimable depuis le portail.
- **Projets (module 08).** Les **projets réels** visibles dans Suivi → Projets proviennent de ce module (avancement, dates, chantier — sans le budget).
- **Magasin / Inventaire (module 09).** Le **catalogue** du portail lit les **produits** de ce module (nom, code, prix, catégorie). Passer une commande **décrémente le stock** et écrit un **mouvement de stock** ici.
- **Comptabilité (module 14).** Le portail **ne génère aucune facture** : les commandes naissent « non payées ». La facturation et l'encaissement se gèrent côté fournisseur.
- **Configuration (module 30).** L'abonnement du fournisseur y est géré. Bon à savoir : **le portail n'est pas affecté** par le mode consultation (lecture seule) que la Configuration peut imposer à l'ERP du fournisseur.
...
Non. Le portail client **ne contient aucun assistant IA** et **ne consomme aucun crédit**. L'assistant « Gestion B2B » est réservé au fournisseur (module 05).
...
- **Deux mondes distincts** : le **portail client** (ce manuel) et le **back-office fournisseur** (module 05, `/ventes?tab=b2b`, administrateurs). C'est le fournisseur qui **approuve** les accès, **chiffre** les soumissions, **crée** les contrats et **fait avancer** les commandes.
...
*Manuels liés :* `05-gestion-crm-opportunites.md` (back-office B2B fournisseur : approbation des accès, liaison des fiches, chiffrage, contrats, statuts de commande, assistant IA B2B), `03-gestion-entreprises.md` (fiche d'entreprise CRM reliée au Suivi), `07-ventes-soumissions.md` (devis réels et page publique du document), `08-ventes-projets.md` (projets réels du Suivi), `09-operations-magasin.md` (produits du catalogue et stock), `14-operations-comptabilite.md` (facturation, hors portail), `30-configuration.md` (abonnement du fournisseur ; le portail n'est pas soumis au mode consultation).

*Manuel ERP Constructo AI — Module 35 « Portail B2B / B2C (clients et fournisseurs) » — v1.0 vérifié — 2026-07.*
